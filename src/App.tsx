...
function calculateTotals(game: Game): Record<string, number> {
  ...
  if (!game.stepsEnabled) return totals
...
function App() {
  ...
  function submitRound(event: FormEvent<HTMLFormElement>) {
    ...
    const victories = finished && !game.finished ? { ...game.victories } : game.victories
    if (finished && !game.finished) {
      const lowest = Math.min(...Object.values(nextTotals))
      for (const player of game.players) if (nextTotals[player.id] === lowest) victories[player.id] = (victories[player.id] ?? 0) + 1
    }
    updateGame({ rounds: nextGame.rounds, finished, victories })
    setRoundOpen(false)
  }
